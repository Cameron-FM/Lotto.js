...
function transformIntoOneArray(wholeSet){
  let newSet = []
...
function mostCommonNumberAcrossWholeSet(singleNumbers){
  ...
  //Count the occurance of each number and push to 2d array
  singleNumbers.forEach((num, i) => {
    ...
    if (count === 1){ //Remove any numbers with just one occurance
      ...
      singleNumbers.splice(i,count) //Once the number and count has been append remove all the remaining occurances from the working list
    }
  })
...
//STEP ONE
let singleNumbers = transformIntoOneArray(rawWinningNums)
console.log(mostCommonNumberAcrossWholeSet(singleNumbers))
